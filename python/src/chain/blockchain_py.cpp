...
void init_blockchain(py::module &m) {
    ...
    .def("most_valuable_addresses", [](Blockchain &chain) {
        std::map<int64_t, AnyScript> ret;
        for (auto &x : mostValuableAddresses(chain)) {
            ret[x.first] = x.second.getScript();
        }
        return ret;
    } , "Get a list of the top 100 most valuable addresses")
    ;

    applyMethodsToSelf(blockchainCl, AddBlockchainMethods{});
}
