...
auto applyMethodsToSelfImpl(F f, std::index_sequence<Is...>, T) {
    return [f](Ty &tx, const std::tuple_element_t<Is + 1, typename T::arg_tuple> &... args) {
        return f(tx, args...);
    };
}
...
auto applyMethodsToSelf(F f) {
    ...
    return applyMethodsToSelfImpl<Ty>(f, std::make_index_sequence<traits::arity - 1>{}, traits{});
}
