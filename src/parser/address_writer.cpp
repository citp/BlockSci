...
scriptFiles(blocksci::apply(blocksci::EquivAddressInfoList(), [&] (auto tag) {
...
void AddressWriter::serializeImp(const ScriptOutput<blocksci::AddressType::PUBKEY> &output, ScriptFile<blocksci::EquivAddressType::PUBKEY> &file) {
    auto data = file.getData(output.scriptNum - 1);
    data->pubkey = output.data.pubkey;
}

void AddressWriter::serializeImp(const ScriptOutput<blocksci::AddressType::WITNESS_SCRIPTHASH> &output, ScriptFile<blocksci::EquivAddressType::SCRIPTHASH> &file) {
    auto data = file.getData(output.scriptNum - 1);
    data->hash256 = output.data.hash;
    data->isSegwit = true;
}

void AddressWriter::serializeImp(const ScriptInput<AddressType::PUBKEYHASH> &input, ScriptFile<EquivAddressType::PUBKEY> &file) {
    auto data = file.getData(input.scriptNum - 1);
    data->pubkey = input.data.pubkey;
}

void AddressWriter::serializeImp(const ScriptInput<AddressType::WITNESS_PUBKEYHASH> &input, ScriptFile<EquivAddressType::PUBKEY> &file) {
    auto data = file.getData(input.scriptNum - 1);
    data->pubkey = input.data.pubkey;
}

void AddressWriter::serializeImp(const ScriptInput<AddressType::WITNESS_SCRIPTHASH> &input, ScriptFile<EquivAddressType::SCRIPTHASH> &file) {
    auto data = file.getData(input.scriptNum - 1);
...
void AddressWriter::serializeImp(const ScriptInput<AddressType::SCRIPTHASH> &input, ScriptFile<EquivAddressType::SCRIPTHASH> &file) {
    auto data = file.getData(input.scriptNum - 1);
    data->wrappedAddress = input.data.wrappedScriptOutput.address();
}
...
void AddressWriter::serializeImp(const ScriptInput<AddressType::NONSTANDARD> &input, ScriptFile<EquivAddressType::NONSTANDARD> &file) {
