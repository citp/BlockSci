...
class AddressWriter {
    ...
    template<blocksci::AddressType::Enum type>
    void serializeImp(const ScriptInput<type> &input, uint32_t addressNum);
...
public:
    ...
    template<blocksci::AddressType::Enum type>
    void serialize(const ScriptInput<type> &input, uint32_t addressNum) {
        serializeImp(input, addressNum);
    }
