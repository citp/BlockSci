...
HashIndexCreator::HashIndexCreator(const ParserConfigurationBase &config) : ParserIndex(config, "hashIndex"), env(blocksci::createHashIndexEnviroment(config.hashIndexFilePath())), wtxn(lmdb::txn::begin(env)), pubkey_dbi(lmdb::dbi::open(wtxn, "pubkey", MDB_CREATE)), scripthash_dbi(lmdb::dbi::open(wtxn, "scripthash", MDB_CREATE)), tx_dbi(lmdb::dbi::open(wtxn, "tx", MDB_CREATE)) {
}
