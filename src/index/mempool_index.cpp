...
namespace blocksci {
    ...
    std::string MempoolIndex::nthTxFilePath(const std::string &baseDirectory, int64_t i) {
        return (boost::filesystem::path{baseDirectory}/(std::to_string(i) + "_tx")).native();
    }
    
    std::string MempoolIndex::nthBlockFilePath(const std::string &baseDirectory, int64_t i) {
