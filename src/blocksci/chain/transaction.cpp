...
namespace blocksci {
    ...
    RawTransaction::RawTransaction(uint32_t sizeBytes_, uint32_t locktime_, uint16_t inputCount_, uint16_t outputCount_, uint16_t vpubCount_) : sizeBytes(sizeBytes_), locktime(locktime_), inputCount(inputCount_), outputCount(outputCount_), vpubCount(vpubCount_) {}
    ...
    Transaction::uint64_range Transaction::vpubold() const {
		auto pos = reinterpret_cast<const char *>(data) + sizeof(RawTransaction) + outputCount() * sizeof(Output) + inputCount() * sizeof(Input);
        /*std::vector<uint64_t> vpub;
        for(int i=0; i < vpubCount(); i++) {
			vpub.emplace_back(reinterpret_cast<uint64_t>(pos));
			pos += sizeof(uint64_t);
		}*/
		return boost::make_iterator_range_n(reinterpret_cast<const uint64_t *>(pos), vpubCount());
		/*std::vector<uint64_t> vpub = access.getVpubold(txNum, vpubCount());
		return vpub;
		return access.getVpubold(txNum, vpubCount());
		std::vector<uint64_t> vpub = access.getVpubold(txNum, vpubCount());
		return boost::iterator_range<uint64_t*>(vpub.begin(), vpub.end());*/
	}
    
	Transaction::uint64_range Transaction::vpubnew() const {
		auto pos = reinterpret_cast<const char *>(data) + sizeof(RawTransaction) + outputCount() * sizeof(Output) + inputCount() * sizeof(Input) + vpubCount() * sizeof(uint64_t);
		/*std::vector<uint64_t> vpub;
        for(int i=0; i < vpubCount(); i++) {
			vpub.emplace_back(reinterpret_cast<uint64_t>(pos));
			pos += sizeof(uint64_t);
		...
		return boost::make_iterator_range_n(reinterpret_cast<const uint64_t *>(pos), vpubCount());
		/*std::vector<uint64_t> vpub = access.getVpubnew(txNum, vpubCount());
		return vpub;
		std::vector<uint64_t> vpub = access.getVpubnew(txNum, vpubCount());
		return boost::iterator_range<uint64_t*>(vpub.begin(), vpub.end());*/
	}
    ...
    bool isJoinsplit(const Transaction &tx) {
			return tx.vpubCount() != 0;
	}
    
    bool isCoinbase(const Transaction &tx) {
        return (tx.inputCount() == 0) && (tx.vpubCount() == 0);
    }
    ...
    uint64_t fee(const Transaction &tx) {
        if (isCoinbase(tx)) {
            ...
            return (totalVpubnew(tx) - totalVpubold(tx)) - (totalOut(tx) - totalIn(tx));
        }
    }
