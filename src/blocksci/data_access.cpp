...
namespace blocksci {
    ...
    DataAccess::DataAccess(const DataConfiguration &config_, bool errorOnReorg_, uint32_t blocksIgnored_) : config(config_), chain(config_, errorOnReorg_, blocksIgnored_), scripts(config_), scriptsFirstSeen(config_) {}
    ...
    DataAccess &DataAccess::Instance(const DataConfiguration &config_, bool errorOnReorg, uint32_t blocksIgnored) {
        ...
        if (!config_.isNull() && config_ != config) {
            ...
            myInstance.scriptsFirstSeen = ScriptFirstSeenAccess(config);
        }
