...
private:
    ...
    T* indirect_ptr(difference_type pos) { return reinterpret_cast<T*>(_union.indirectAccess.indirect) + pos; }
    const T* indirect_ptr(difference_type pos) const { return reinterpret_cast<const T*>(_union.indirectAccess.indirect) + pos; }
    ...
    void change_capacity(size_type new_capacity) {
        if (new_capacity <= N) {
            ...
            if (!is_direct()) {
                _union.indirectAccess.indirect = static_cast<char*>(realloc(_union.indirectAccess.indirect, ((size_t)sizeof(T)) * new_capacity));
                _union.indirectAccess.capacity = new_capacity;
            } else {
                char* new_indirect = static_cast<char*>(malloc(((size_t)sizeof(T)) * new_capacity));
                T* src = direct_ptr(0);
                T* dst = reinterpret_cast<T*>(new_indirect);
                memcpy(dst, src, size() * sizeof(T));
                _union.indirectAccess.indirect = new_indirect;
                _union.indirectAccess.capacity = new_capacity;
                _size += N + 1;
            }
        }
...
public:
    ...
    size_t capacity() const {
        if (is_direct()) {
            ...
            return _union.indirectAccess.capacity;
        }
    }
    ...
    ~prevector() {
        clear();
        if (!is_direct()) {
            free(_union.indirectAccess.indirect);
            _union.indirectAccess.indirect = NULL;
        }
    }
