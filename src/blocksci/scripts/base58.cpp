...
namespace blocksci {
    ...
    CBitcoinAddress::CBitcoinAddress(const uint160 &dest, ScriptType::Enum type, const DataConfiguration &config) {
        if (type == ScriptType::Enum::PUBKEYHASH || type == ScriptType::Enum::PUBKEY) {
            SetData(config.pubkeyPrefix, &dest, sizeof(dest));
        } else if (type == ScriptType::Enum::SCRIPTHASH) {
    ...
    std::pair<uint160, ScriptType::Enum> CBitcoinAddress::Get(const DataConfiguration &config) const
    {
        ...
        if (vchVersion == config.pubkeyPrefix)
            return std::make_pair(id, ScriptType::Enum::PUBKEYHASH);
