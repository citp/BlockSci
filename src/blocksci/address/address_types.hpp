...
#define EQUIV_ADDRESS_TYPE_LIST VAL(NONSTANDARD), VAL(PUBKEY), VAL(MULTISIG_PUBKEY), VAL(SCRIPTHASH), VAL(MULTISIG), VAL(NULL_DATA)
#define EQUIV_ADDRESS_TYPE_SET VAL(NONSTANDARD) VAL(PUBKEY) VAL(MULTISIG_PUBKEY) VAL(SCRIPTHASH) VAL(MULTISIG) VAL(NULL_DATA)
...
namespace blocksci {
    ...
    struct EquivAddressType {
        ...
        #define VAL(x) Enum::x
        static constexpr std::array<Enum, 6> all = {{EQUIV_ADDRESS_TYPE_LIST}};
        #undef VAL
