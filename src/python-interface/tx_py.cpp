...
void init_tx(py::module &m) {
    ...
    .def_property_readonly("vpubold", &Transaction::vpubold, "A list of the vpub_old values of the transaction")
    .def_property_readonly("vpubnew", &Transaction::vpubnew, "A list of the vpub_new values of the transaction")
    ...
    .def_property_readonly("sum_vpubold", py::overload_cast<const Transaction &>(totalVpubold), "The sum of the value of all of the vpubold values")
    .def_property_readonly("sum_vpubnew", py::overload_cast<const Transaction &>(totalVpubnew), "The sum of the value of all of the vpubnew values")
    ...
    .def_property_readonly("is_joinsplit", isJoinsplit, "Return's true if this transaction is a Joinsplit transaction")
    .def_property_readonly("joinsplits", &Transaction::vpubCount, "The number of JoinSplits in the transaction")
    ...
    py::class_<Transaction::uint64_range>(m, "Uint64Range", "Class representing a range of long unsigned ints")
    .def("__len__", &Transaction::uint64_range::size)
    .def("__iter__", [](const Transaction::uint64_range &range) { return py::make_iterator(range.begin(), range.end()); },
         py::return_value_policy::reference, py::keep_alive<0, 1>())
    .def("__getitem__", [](const Transaction::uint64_range &range, int64_t i) -> const uint64_t & {
        while (i < 0) {
            i = range.size() - i;
        }
        uint64_t posIndex = static_cast<uint64_t>(i);
    ...
    .def("__getitem__", [](const Transaction::uint64_range &range, py::slice slice) -> py::list {
        size_t start, stop, step, slicelength;
        if (!slice.compute(range.size(), &start, &stop, &step, &slicelength))
            throw py::error_already_set();
        py::list txList;
